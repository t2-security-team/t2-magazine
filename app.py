...
with st.sidebar:
    ...
    st.divider()
    vis_option = st.radio("🎨 시각화 옵션", ["적용 안 함", "1. ✈️ 항공사별 색상 표시 (DL:연하늘, OZ:연분홍)", "2. ⏰ 첨두시간 색상 표시 (16~18시)"], index=0)
...
if not (p_all and g_all):
    ...
    with st.expander("💡 홈페이지 이용 방법 (필독)", expanded=True):
        st.markdown("""
        ### 🌐 데이터 공유 방식 안내
        * **자동 공유:** 1번째 파일(승객수 파일)을 업로드하면 구글 시트에 안전하게 보관되며, **모든 팀원이 동일한 데이터를 볼 수 있습니다.**
        * **비우기 버튼:** 다음 날 데이터를 넣기 전, 사이드바의 **[🗑️ 전체 데이터 비우기]** 버튼을 누르면 서버 데이터가 깔끔하게 초기화됩니다.
        """)
else:

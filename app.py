...
if not (p_all and g_all):
    ...
    with st.expander("📢 8월 15일 시스템 업데이트 안내 (필독)", expanded=True):
        st.markdown("""
        **1. 야간 근무자를 위한 '오늘 자 실시간 잡지' 유지 기능 추가**
        현장의 소중한 의견을 적극 반영하여, 내일 자 승객 데이터를 미리 올리더라도 당일 자정(00시)까지는 오늘 자 잡지를 계속 조회할 수 있도록 시스템을 개선했습니다.
        ...
        **2. 데이터 업로드 시 매니저님 유의사항**
        업로드 방식은 기존과 크게 다르지 않습니다. 승객수 파일 업로드 시, 상단의 업로드 날짜 선택란에 **'내일'**이 올바르게 체크되어 있는지만 한 번 더 확인하신 후 등록해 주시면 됩니다!
        """)
else:
